...
test("resolves with the value when the promise settles in time", async () => {
  await expect((0, _withTimeout.withTimeout)(Promise.resolve("ok"), 1000, "fetch")).resolves.toBe("ok");
});
test("rejects when the promise does not settle within the timeout", async () => {
  const never = new Promise(() => {});
  await expect((0, _withTimeout.withTimeout)(never, 10, "fetch")).rejects.toThrow(/fetch did not resolve within 10ms/);
});
test("passes the promise through unchanged when the timeout is disabled", async () => {
  await expect((0, _withTimeout.withTimeout)(Promise.resolve("ok"), 0, "fetch")).resolves.toBe("ok");
});
